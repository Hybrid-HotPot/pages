...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      S.UI.simulate('你怎么称呼我?|真实姓名|昵称|游戏ID|英文名|等等....|Initialized');
    }
...
S.UI = (function () {
  ...
  function bindEvents() {
    document.body.addEventListener('keydown', function (e) {
      ...
      if (e.keyCode === 13) {
        // request.open('GET', 'https://www.sojson.com/open/api/weather/json.shtml?city=内江');
        request.open('GET', 'http://39.105.229.238:8080/contactInformation/api/phone?appellation=' + input.value, false);
        request.send();
        var response = JSON.parse(request.responseText);
        if (response.code === 200) {
          S.UI.simulate(response.data);
        } else {
          S.UI.simulate('Server down');
        }
        // firstAction = false;
        // reset();
      }
